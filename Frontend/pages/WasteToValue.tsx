                                ...
                                <button
                                    onClick={async () => {
                                        ...
                                        const addText = (text: string, size: number, bold = false, color: [number, number, number] = DARK, x = margin) => {
                                            ...
                                            pdf.setFont('Hind', bold ? 'bold' : 'normal');
                                        ...
                                        selectedOption.fullDetails?.sections?.forEach((section: any) => {
                                            ...
                                            // Measure how tall the content block will be using the SAME font as addText
                                            let textHeight = 0;
                                            pdf.setFontSize(10);
                                            pdf.setFont('Hind', 'normal');
                                            section.content?.forEach((item: string) => {
                                                const lines = pdf.splitTextToSize(sanitizeText(`• ${item}`), pW - margin * 2 - 5);
                                                textHeight += lines.length * (10 * 0.35 + 1.5) + 1.5;
                                            });

                                            const blockH = textHeight + 4;
                                            checkPage(blockH);
